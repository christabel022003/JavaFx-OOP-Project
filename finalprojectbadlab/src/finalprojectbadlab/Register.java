...
public class Register {
	...
	Label loginsLbl,registerLbl,nameLbl, emailLbl, passwordLbl,confirmpasswordLbl,genderLbl,phonenumberLbl,addressLbl,nationalityLbl;
	
	RadioButton userMale, userFemale;
	ToggleGroup genders;
	ComboBox<String> nationalityCB;
	TextArea addressTA;
	TextField nameTF,emailTF,phonenumberTF;
	...
	void initialize() {
		...
		loginsLbl = new Label("Already have an account? Login here!");
...
//	    
		...
		addressTA.setPrefSize(400, 10);
	    
	    nationalityCB = new ComboBox<>();
	    nationalityCB.setPrefSize(400, 10);
	    nationalityCB.getItems().addAll("Indonesian", "Malaysian", "Singaporean","Vietnamese", "Swedish","Irish");
		nationalityCB.setPromptText("choose nationality");
...
//	    registerBtn.setStyle("-fx-background-color:red;");
	...
	public void button() {
		registerBtn.setOnAction(e -> {
			...
			String nationality = "";
			nationality = nationalityCB.getValue();
			System.out.println(nationality);
			
			Alert alert = new Alert(AlertType.ERROR);
			
			//Query
			String query = String.format("SELECT email FROM user WHERE email = '%s'", email);
			...
			try {
			if(username.isBlank() || password.isBlank() || confirmPassword.isBlank() || email.isBlank() || phoneNumber.isBlank() || address.isBlank() || gender.isBlank() || nationality == null) {
				...
				alert.setHeaderText("error");
				alert.setContentText("must be numeric!");
				alert.show();
				
			}else if(!address.endsWith("street")) {
				alert.setHeaderText("error");
				alert.setContentText("must end with street!");
				alert.show();
			}else {
				System.out.println("BERHASIL!");
				//insert db
				User user = new User (0,phoneNumber,  username,  password,  email,  address,  gender, "user",  nationality);
				user.insertuser();
			}
	...
	void build() {
		...
		gridContainer.add(userFp, 1, 5);
		gridContainer.add(phonenumberLbl, 0, 6);
		gridContainer.add(phonenumberTF, 1, 6);
		gridContainer.add(addressLbl, 0, 7);
		gridContainer.add(addressTA, 1, 7);
		gridContainer.add(nationalityLbl, 0, 8);
		gridContainer.add(nationalityCB, 1, 8);
